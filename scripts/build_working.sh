...
# Working Build Script for GPU Kernel Project
# Builds only the kernels that are known to work reliably
...
echo "=== Working GPU Kernel Project Build ==="
...
# Step 2: Build working kernels
print_status "Step 2: Building working kernels..."

# Create build directory for kernels
mkdir -p build_simple/bin

# Build vector addition (simple, reliable)
...
echo ""
echo "Note: Some kernels (warp_primitives, advanced_fft, nbody_simulation) are excluded"
echo "due to compatibility issues. See UNIFIED_BUILD_SUMMARY.md for details."

if [ $KERNEL_COUNT -eq $EXPECTED_KERNELS ] && [ "$GUI_BUILT" = true ]; then
    print_success "All working components built successfully!"
    echo ""
    echo "To run the GUI:"
